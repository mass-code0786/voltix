...
async function main() {
  for (const coin of coinCatalog) {
    ...
    await prisma.coinMetadata.upsert({ where:{symbol:coin.symbol}, update:{name:coin.name,pair,isActive:coin.enabled!==false,displayOrder,logoUrl:coin.logoUrl,localLogoPath}, create:{symbol:coin.symbol,name:coin.name,pair,isActive:coin.enabled!==false,displayOrder,logoUrl:coin.logoUrl,localLogoPath} });
  }
  ...
  for (const [label, utcTime] of [["Window 1", "08:30"], ["Window 2", "12:30"], ["Window 3", "19:50"]]) {
