...
export function AppHeader({ title, subtitle, compactBrand = true, initials, unreadNotifications = 0, onMenuButton, onNotifications, onMenu }: HeaderProps) {
  return (
    <header className="sticky top-0 z-30 h-[58px] bg-[#060a08]/72 px-4 backdrop-blur-2xl lg:h-[62px] lg:border-b lg:border-white/[.06] lg:px-8">
      <div className="mx-auto flex h-[58px] max-w-[420px] items-center justify-between gap-1.5 lg:h-[62px] lg:max-w-6xl">
        <div className="flex min-w-0 items-center gap-1.5 lg:hidden">
          <button onClick={onMenuButton ?? onMenu} className="grid h-[42px] w-[42px] shrink-0 place-items-center rounded-[15px] border border-white/[.08] bg-white/[.035] text-white shadow-[0_10px_28px_rgba(0,0,0,.28),inset_0_1px_0_rgba(255,255,255,.08)]" aria-label="Open menu">
            <Menu size={18} />
          </button>
        ...
        <div className="flex shrink-0 items-center gap-1.5">
          <button onClick={onNotifications} className="relative grid h-[42px] w-[42px] place-items-center rounded-full border border-white/[.08] bg-white/[.045] text-slate-200 shadow-[0_0_24px_rgba(24,255,138,.1),inset_0_1px_0_rgba(255,255,255,.08)]" aria-label="Notifications">
            <Bell size={18} />
          ...
          <button onClick={onMenu} className="relative grid h-[46px] w-[46px] place-items-center rounded-full border border-[#18ff8a]/55 bg-[linear-gradient(145deg,#dfffe8,#18ff8a_55%,#008f55)] text-xs font-black text-[#050608] shadow-[0_14px_30px_rgba(24,255,138,.22),inset_0_1px_0_rgba(255,255,255,.45)]" aria-label="Account menu">
            {initials}
...
export function BottomNav<T extends string>({ items, activeId, activeSection, labelFor, onSelect }: BottomNavProps<T>) {
  return (
    <nav className="fixed inset-x-0 bottom-[14px] z-50 px-4 lg:hidden">
      <div className="mx-auto flex h-[72px] max-w-[390px] items-center justify-around rounded-full border border-white/[.16] bg-[#0a1013]/82 px-2 shadow-[0_-14px_42px_rgba(0,0,0,.48),0_0_26px_rgba(24,255,138,.12),inset_0_1px_0_rgba(255,255,255,.08)] backdrop-blur-2xl">
        {items.map(({ id, label, icon: Icon, section }) => {
          ...
          return (
            <button key={`${id}-${section ?? label}`} onClick={() => onSelect(id, section)} aria-current={active ? "page" : undefined} className={`flex min-w-0 flex-1 flex-col items-center justify-center gap-0.5 text-[11px] font-black transition ${active ? "text-[#18ff8a]" : "text-slate-500"}`}>
              <span className={`${center ? "-mt-8 h-[60px] w-[60px] rounded-full border-[#18ff8a]/35 bg-[radial-gradient(circle_at_35%_25%,#dfffe8,#18ff8a_45%,#047a48)] text-[#050608] shadow-[0_0_28px_rgba(24,255,138,.56)]" : `h-8 w-9 rounded-2xl ${active ? "border-[#18ff8a]/25 bg-[#18ff8a]/10 shadow-[0_0_20px_rgba(24,255,138,.16)]" : "border-transparent bg-transparent"}`} grid place-items-center border transition`}>
                {center ? <span className="text-xl font-black">V</span> : <Icon size={22} strokeWidth={active ? 2.5 : 1.9} />}
              </span>
