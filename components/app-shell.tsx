...
function CoinRow({ coin, action }: { coin: AppCoin&{volume?:number;live?:boolean}; action?: () => void }) {
  ...
  return <button onClick={action} className="grid w-full grid-cols-[1fr_auto_auto] items-center gap-3 px-5 py-4 text-left hover:bg-white/[.025]"><div className="flex min-w-0 items-center gap-3"><CoinMark symbol={coin.symbol} color={coin.color} logoPath={coin.localLogoPath} /><div><div className="font-bold">{coin.symbol}<span className="ml-1.5 text-[10px] font-normal text-slate-500">/USDT</span></div><p className="mt-1 text-xs text-slate-500">{coin.name}</p><p className="mt-1 text-[9px] text-slate-600">24h vol {coin.live&&coin.volume!==undefined?compact(coin.volume):"Waiting"}</p></div></div><div className="hidden sm:block"><Sparkline data={coin.spark} positive={coin.change >= 0} /></div><div className="min-w-[88px] text-right"><p className="text-sm font-bold">{coin.live?`$${shown}`:"Waiting"}</p>{coin.live&&<p className="mt-1 text-[11px] text-slate-500">?{(coin.price * usdInr).toLocaleString("en-IN", { maximumFractionDigits: coin.price < 1 ? 4 : 2 })}</p>}<p className={`mt-1 text-xs font-bold ${coin.change >= 0 ? "text-mint" : "text-danger"}`}>{coin.live?`${coin.change >= 0 ? "+" : ""}${coin.change.toFixed(2)}%`:"Live"}</p></div></button>;
}
...
function TradeMenu({close,select}:{close:()=>void;select:(category:TradeCategory)=>void}) {
  const options: {id:TradeCategory;label:string;description:string;icon:typeof Home}[] = [
    ...
    {id:"copy",label:"Copy Trading",description:"Open Bitex copy strategies",icon:Zap},
  ];
...
function WalletTransferModal({initialFrom,initialTo,balances,close,transfer}:{initialFrom:UserWallet;initialTo:UserWallet;balances:Record<UserWallet,number>;close:()=>void;transfer:(from:UserWallet,to:UserWallet,amount:number)=>boolean}) {
  ...
  const label=(wallet:UserWallet)=>`${wallet[0]}${wallet.slice(1).toLowerCase()} Wallet`;
