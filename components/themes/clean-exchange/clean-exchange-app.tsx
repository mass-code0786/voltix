...
function CleanHeader({ routes, section, unreadCount }: { routes: Record<CleanSection, string>; section: CleanSection; unreadCount: number }) {
  const recordsHref = section === "ai-trade" ? "/dashboard?view=aiTrade&workflow=current" : "/dashboard?view=wallet&wallet=ledger&workflow=current";
  return <header className={styles.header}>
    <Link href={routes.home} className={styles.brand} aria-label="Voltix home"><span>V</span><b>VOLTIX</b></Link>
    <div className={styles.headerActions}>
      {section === "home" && <><Link href="/dashboard?view=markets&workflow=current" aria-label="Search"><Search /></Link><Link href="/wallet/deposit" aria-label="Scan"><QrCode /></Link><Link href="/profile/support" aria-label="Support"><Headphones /></Link></>}
      {section === "markets" && <><Link href="/dashboard?view=markets&workflow=current" aria-label="Search"><Search /></Link><button type="button" aria-label="Favorites are unavailable" disabled><Star /></button></>}
      {section === "futures" && <><Link href="/dashboard?view=markets&workflow=current" aria-label="Search"><Search /></Link><Link href={recordsHref} aria-label="Records"><FileClock /></Link><Link href={recordsHref} aria-label="Recent activity"><History /></Link></>}
...
function AiPage({ data, markets, loading, visible, onToggle }: { data: Snapshot; markets: Market[]; loading: boolean; visible: boolean; onToggle: () => void }) {
  ...
  return <>
    ...
    <section className={styles.aiCta}><div><h2>How AI Trade Works?</h2><Link href="/dashboard?view=aiTrade&workflow=current">Learn More</Link><p>Start New AI Trade</p><small>Find best market opportunity</small></div><Link href="/dashboard?view=aiTrade&workflow=current" aria-label="Start New AI Trade"><ChevronRight /></Link></section>
  </>;
}
...
function FuturesPage({ data, markets, loading, marketsHref: _marketsHref }: { data: Snapshot; markets: Market[]; loading: boolean; marketsHref: string }) {
  ...
  return <>
    ...
    <div className={styles.futuresActions}><ActionItem icon={ArrowLeftRight} label="Transfer" href="/dashboard?view=wallet&action=transfer&workflow=current" /><ActionItem icon={RefreshCw} label="Convert" href="/markets/SHINEUSDT" /><ActionItem icon={Wallet} label="Borrow" disabled /><ActionItem icon={ArrowDownToLine} label="Repay" disabled /><ActionItem icon={History} label="History" href="/dashboard?view=wallet&wallet=ledger&workflow=current" /><ActionItem icon={CircleDollarSign} label="Funding" disabled /></div>
...
function QuickActions({ compact = false }: { compact?: boolean }) {
  const actions: Array<[string, string, ComponentType<{ size?: number }>, string]> = [["Deposit", "/wallet/deposit", ArrowDownToLine, "deposit"], ["Withdraw", "/wallet/withdraw", ArrowUpFromLine, "withdraw"], ["Transfer", "/dashboard?view=wallet&action=transfer&workflow=current", ArrowLeftRight, "transfer"], [compact ? "More" : "History", compact ? "/profile" : "/dashboard?view=wallet&wallet=ledger&workflow=current", compact ? MoreHorizontal : History, "history"]];
...
function WalletActions() {
  const actions: Array<[string, string, ComponentType<{ size?: number }>]> = [["Deposit", "/wallet/deposit", ArrowDownToLine], ["Withdraw", "/wallet/withdraw", ArrowUpFromLine], ["Transfer", "/dashboard?view=wallet&action=transfer&workflow=current", ArrowLeftRight], ["Convert", "/markets/SHINEUSDT", Zap], ["History", "/dashboard?view=wallet&wallet=ledger&workflow=current", History]];
...
function Stat({ icon: Icon, label, value, href }: { icon: ComponentType<{ size?: number }>; label: string; value: string; href?: string }) { const content = <><Icon /><b>{label}</b><small>{value}</small></>; return <div>{href ? <Link href={href}>{content}</Link> : content}</div>; }
...
function walletDetailsHref(key: string) { if (key === "ai") return "/dashboard?view=aiTrade&workflow=current"; if (key === "reward") return "/markets/SHINEUSDT"; return "/dashboard?view=wallet&wallet=assets&workflow=current"; }
