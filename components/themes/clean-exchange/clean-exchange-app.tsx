...
export default function CleanExchangeApp({ section, routeMode = "preview" }: { section: CleanSection; routeMode?: "preview" | "production" }) {
  ...
  return <main className={styles.app}>
    <div className={styles.phone}>
      <CleanHeader routes={routes} />
      <div className={`${styles.content} ${styles[`${section.replace("-", "")}Page`] ?? ""}`} aria-busy={loading}>
        {section === "home" && <HomePage data={data} markets={markets} loading={loading} routes={routes} />}
...
function CleanHeader({ routes }: { routes: Record<CleanSection, string> }) {
  return <header className={styles.header}>
    <Link href={routes.home} className={styles.brand} aria-label="Voltix home"><span>V</span><b>VOLTIX</b></Link>
    <div className={styles.headerActions}><Link href={routes.markets} aria-label="Search markets"><Search /></Link><Link href="/dashboard?view=wallet&wallet=ledger" aria-label="Wallet history"><History /></Link><Link href="/profile/notifications" aria-label="Notifications"><Bell /></Link></div>
  </header>;
}

function HomePage({ data, markets, loading, routes }: { data: Snapshot; markets: Market[]; loading: boolean; routes: Record<CleanSection, string> }) {
  ...
  return <>
    ...
    <Link href={routes["ai-trade"]} className={styles.promo}><div><span className={styles.promoIcon}><Bot /></span><h2>AI Copy Trading</h2><p>Use your active Voltix AI trade windows.</p><b>View AI Trade <ChevronRight /></b></div><Bot className={styles.promoBot} /></Link>
    ...
    <div className={styles.walletStrip}>{walletRows(data).map(row => <div className={styles.walletMini} key={row.key}><Wallet /><span>{row.label}</span>{loading ? <i className={`${styles.skeleton} ${styles.inlineSkeleton}`} /> : <b>{money(row.value)}</b>}</div>)}</div>
...
function WalletPage({ data, loading }: { data: Snapshot; loading: boolean }) {
  ...
  return <>
    ...
    <div className={styles.walletList}>{loading ? <SkeletonWallets /> : rows.map(row => <Link key={row.key} href="/dashboard?view=wallet"><span className={styles.walletIcon}><row.icon /></span><div><b>{row.label} Wallet</b><small>Available</small></div><strong>{money(row.value)}</strong><ChevronRight /></Link>)}</div>
