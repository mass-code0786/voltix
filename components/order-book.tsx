...
export function BuySellPressurePanel({symbol="BTCUSDT"}:{symbol?:string}){
  ...
  useEffect(()=>{
    ...
    const connect=()=>{
      ...
      source.addEventListener("open",()=>{attempt=0;void loadTrades();});
      ...
      source.addEventListener("error",event=>{
        console.error("[market-data] pressure stream disconnected",event);
        source?.close();
        attempt+=1;
    ...
    void loadTrades();connect();
    return()=>{active=false;if(reconnectTimer)clearTimeout(reconnectTimer);source?.close();};
  },[symbol]);
  const pressure=useMemo(()=>{const recent=trades.slice(0,30);const buy=recent.filter(trade=>trade.side==="BUY").reduce((sum,trade)=>sum+trade.quoteQty,0);const sell=recent.filter(trade=>trade.side==="SELL").reduce((sum,trade)=>sum+trade.quoteQty,0);const total=Math.max(buy+sell,1);return {buyPct:(buy/total)*100,sellPct:(sell/total)*100};},[trades]);
  return <section className="orderbook-pressure"><div><span>Buy/Sell Pressure</span><b>{pressure.buyPct.toFixed(0)}% / {pressure.sellPct.toFixed(0)}%</b></div><div className="flex h-2 overflow-hidden rounded-full bg-[#ff4f6d]/20"><i className="bg-[#18ff8a]" style={{width:`${pressure.buyPct}%`}}/><i className="bg-[#ff4f6d]" style={{width:`${pressure.sellPct}%`}}/></div></section>;
}
...
export function OrderBookPanel({symbol="BTCUSDT"}:{symbol?:string}){
  ...
  useEffect(()=>{
    ...
    const connect=()=>{
      source?.close();
      source=new EventSource("/api/prices/stream");
      source.addEventListener("open",()=>{attempt=0;void loadBook();void loadTicker();});
      ...
      source.addEventListener("error",event=>{
        ...
        const delay=Math.min(30000,1000*2**Math.min(attempt-1,5));
        if(reconnectTimer)clearTimeout(reconnectTimer);
        void loadBook();void loadTicker();
        reconnectTimer=setTimeout(()=>{if(active)connect();},delay);
      });
    };
    void loadBook();void loadTicker();connect();
    return()=>{active=false;if(reconnectTimer)clearTimeout(reconnectTimer);source?.close();};
  },[symbol]);
