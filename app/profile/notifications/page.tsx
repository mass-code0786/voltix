...
export default function NotificationsPage(){
  ...
  return <main className="profile-page min-h-screen px-4 py-4 text-white sm:px-6"><div className="mx-auto max-w-2xl"><ProfilePageHeader title="Notifications" icon={Bell} subtitle={unread?`${unread} unread`:"All caught up"}/><section className="profile-glass mt-4 rounded-[22px] p-4">{unread>0&&<button onClick={markRead} className="mb-3 w-full rounded-2xl border border-[#18ff8a]/25 bg-[#18ff8a]/10 py-3 text-xs font-black text-[#18ff8a]">Mark as read</button>}{error&&<p className="text-xs text-[#ff4f6d]">{error}</p>}<div className="space-y-2">{items.length?items.map(n=>{const target=notificationTarget(n);const details=promotionNotificationDetails(n.metadata);return <div key={n.id} onClick={()=>{if(target)router.push(target);}} className={`rounded-2xl border border-white/[.08] p-3 ${target?"cursor-pointer":""} ${n.unread?"bg-[#18ff8a]/10":"bg-black/25"}`}><div className="flex items-start gap-3"><span className={`mt-1 h-2 w-2 rounded-full ${n.unread?"bg-[#18ff8a]":"bg-slate-700"}`}/><div className="min-w-0 flex-1"><p className="font-bold text-white">{n.title}</p><p className="mt-1 text-xs leading-5 text-slate-400">{n.message}</p>{details.length>0&&<div className="mt-2 space-y-0.5 text-[10px] text-slate-500">{details.map(detail=><p key={detail}>{detail}</p>)}</div>}<p className="mt-2 text-[10px] text-slate-600">{new Date(n.createdAt).toLocaleString()}</p></div></div></div>}):<p className="rounded-2xl border border-white/[.08] bg-black/25 p-6 text-center text-sm text-slate-500">No notifications</p>}</div></section></div></main>;
}
