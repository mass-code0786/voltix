...
export default function SettingsPage(){
  ...
  return <Frame><form onSubmit={save} className="profile-glass rounded-[22px] p-4"><div className="space-y-3"><SearchableSelect label="Language" options={languageOptions} value={language} onChange={setLanguage} placeholder="Search language"/><fieldset className="appearance-picker rounded-2xl border border-white/[.08] bg-black/25 p-3"><legend className="px-1 text-[11px] font-bold uppercase tracking-wider text-slate-500">Appearance</legend><div className="mt-1 grid gap-2">{themes.map(theme=><label key={theme.value} className={`appearance-option ${preference===theme.value?"is-selected":""}`}><input type="radio" name="theme" value={theme.value} checked={preference===theme.value} onChange={()=>setPreference(theme.value)}/><span><b>{theme.label}</b><small>{theme.description}</small></span></label>)}</div></fieldset><div className="rounded-2xl border border-white/[.08] bg-black/25 p-3"><p className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Account preferences</p><p className="mt-1 text-sm text-slate-400">Core account preferences are synced with your profile.</p></div></div>{(error||message)&&<p className={`mt-3 text-xs font-bold ${error?"text-[#ff4f6d]":"text-[#18ff8a]"}`}>{error||message}</p>}<button disabled={saving||!profile} className="mt-4 w-full rounded-2xl bg-[#18ff8a] py-3.5 text-sm font-black text-[#050608] disabled:opacity-60">{saving?"Saving...":"Save Settings"}</button></form></Frame>;
}
